...
resource "azurerm_windows_virtual_machine" "TF_VM4testingNetwork_OnPremises" {
  ...
  tags = {
    usage = "On prem Windows VM for testing network connectivity"
  }
...
## a Windows Server 2022 VM connected to the Hub Vnet for testing network connectivity
# cf. https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/windows_virtual_machine

resource "azurerm_windows_virtual_machine" "TF_VM4testingNetwork_HubVnet" {
  count                 = var.EnableVM4testingNetwork_HubVnet
  name                  = "VMHubVnet"
  resource_group_name   = azurerm_resource_group.TF_RG.name
  location              = azurerm_resource_group.TF_RG.location
  size                  = var.AzureVMSize    # B2ms or greater is better for good experience
  admin_username        = var.AdminUsername
  admin_password        = var.AdminPassword
  network_interface_ids = [azurerm_network_interface.TF_VM4testingNetwork_HubVnet_Nic[0].id]
  os_disk {
    caching              = "ReadWrite"
    storage_account_type = "Standard_LRS"
  }
  ...
  computer_name = "VMHubVnet"
  provision_vm_agent = true
  timezone = "W. Europe Standard Time"
  tags = {
    usage = "VM in Hub Vnet for testing network connectivity"
  }
...
resource "azurerm_network_interface" "TF_VM4testingNetwork_HubVnet_Nic" {
  count               = var.EnableVM4testingNetwork_HubVnet
  name                = "VMHubVnet-Nic"
  location            = azurerm_resource_group.TF_RG.location
  resource_group_name = azurerm_resource_group.TF_RG.name
